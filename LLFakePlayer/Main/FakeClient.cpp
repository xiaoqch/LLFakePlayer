...
#ifdef DEBUG
//static_assert(PlayerOffset::mSimulatedOldY == offsetof(FakeSimulatedPlayer, mOldY));
#endif // DEBUG
...
void handle(SimulatedPlayer* sp, PlayStatusPacket* packet)
{
    ...
    if (Config::ChunkRadius < 7)
        logger.debug("ChunkRadius should be greater than or equal to 7 to ensure that there are enough chunks (5*5) to change dimension");

    sp->setChunkRadius(Config::ChunkRadius);

    // ================== Fix Region (Desperated) ==================
...
TClasslessInstanceHook(void, "?sendToClients@LoopbackPacketSender@@UEAAXAEBV?$vector@UNetworkIdentifierWithSubId@@V?$allocator@UNetworkIdentifierWithSubId@@@std@@@std@@AEBVPacket@@@Z",
                       std::vector<NetworkIdentifierWithSubId> const& clients, class Packet& packet)
{
    // DEBUGL("[sendToClients] -> {} clients: {}({})", clients.size(), packet.getName(), packet.getId());
...
// ================= Fix SimulatedPlayer =================
extern void tickFakeSimulatedPlayer(SimulatedPlayer& sp);
...
#ifdef VERBOSE
        DEBUGL("Dimension: {}, cx range:[{}:{}], cz range:[{}:{}], Info: {}",
               region.getDimensionId(), min_cx, max_cx, min_cz, max_cz, ColorFormat::convertToColsole(loadInfo.first));
#endif // VERBOSE
...
{
    DEBUGL("ServerPlayer({})::prepareRegion(ChunkSource(dimid: {}))",
           this->getNameTag(), (int)cs.getDimension().getDimensionId());
    original(this, cs);
}
